...
function query_open_group_classes() {
  $sql = function($conn) {
    return "SELECT gc.*, GROUP_CONCAT(rc.email ORDER BY rc.group_class_id) AS regulars FROM group_class gc LEFT JOIN regular_client rc ON gc.id=rc.group_class_id WHERE NOW() < gc.end OR gc.end IS NULL GROUP BY gc.id";
  };
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <input type="radio" name="type" value="normal" <?= $group_class['type'] == 'normal' ? 'checked' : '' ?>>Normaali</input>
        <input type="radio" name="type" value="miniretreat" <?= $group_class['type'] == 'miniretreat' ? 'checked' : '' ?>>Miniretriitti</input>
        <input type="radio" name="type" value="course" <?= $group_class['course'] == 'normal' ? 'checked' : '' ?>>Kurssi</input>
      </td>
    </tr>
    <?php endforeach; ?>
  </tbody>
